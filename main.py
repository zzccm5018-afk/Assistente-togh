...
Variáveis de ambiente (.env / Railway -> Variables):
  ...
  Proteção da API
    API_TOKEN                  token exigido no /chat (Authorization: Bearer <token>)
...
Novidades desta versão:
    ...
    - /chat protegido por token (API_TOKEN).
...
def check_token(authorization: Optional[str] = Header(None)) -> None:
    """Exige 'Authorization: Bearer <API_TOKEN>'. Sem API_TOKEN definido, a proteção fica desligada."""
