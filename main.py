...
# Quantas vezes tenta de novo quando a Gemini responde 503/instável, antes de desistir.
...
def _gemini_http_error(exc: Exception, what: str) -> HTTPException:
    ...
    if code in (500, 503) or "UNAVAILABLE" in msg.upper():
        return HTTPException(
            ...
            f"Já tentamos de novo automaticamente; tente novamente em instantes.",
        )
...
def _is_transient_gemini_error(exc: Exception) -> bool:
    ...
    valem uma nova tentativa. Erros de chave inválida ou cota estourada NÃO entram
    aqui, porque tentar de novo não resolve.
    """
    code = getattr(exc, "code", None)
    msg = str(exc).upper()
    if code in (500, 503):
...
def _generate_sync(model: str, system_prompt: str, history: List[Dict[str, str]], user_message: str) -> str:
    """
    Chama a Gemini para gerar a resposta. Se o erro for passageiro (503/instabilidade
    do provedor), tenta de novo automaticamente até GEMINI_MAX_RETRIES vezes, com
    espera crescente entre as tentativas (1s, 2s, 4s, ...). Erros definitivos (chave
    inválida, cota estourada) sobem na primeira falha, sem retry.
    """
