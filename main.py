...
Variáveis de ambiente (.env / Railway -> Variables):
  ...
  Opcionais (têm padrão)
    GEMINI_CHAT_MODEL          padrão "gemini-2.5-flash-lite"
    ...
    GEMINI_MAX_RETRIES         padrão "2"   (novas tentativas em erro transitório: 503/429)
    GEMINI_RETRY_BASE_DELAY    padrão "1.5" (segundos; dobra a cada tentativa - backoff exponencial)
    PORT                       padrão "8000" (só usado ao rodar `python main.py`)

  IMPORTANTE sobre o modelo ativo (tabela model_config no Supabase):
    Aliases terminados em "-latest" (ex: "gemini-flash-lite-latest") apontam sempre para a
    versão mais nova do Google e por isso tendem a sofrer mais picos de indisponibilidade
    (503 Service Unavailable) logo após serem lançados. Se você estiver vendo 503 com
    frequência, considere trocar o active_model na tabela model_config para uma versão
    "pinada" e estável, por exemplo "gemini-2.5-flash-lite" ou "gemini-3.1-flash-lite".
    O retry automático abaixo ajuda a absorver picos pontuais, mas não substitui escolher
    um modelo mais estável se a instabilidade for recorrente.
...
Novidades desta versão:
    ...
    - NOVO: retry automático com backoff exponencial para erros transitórios da Gemini
      (503 Service Unavailable e 429 RESOURCE_EXHAUSTED), tanto na geração de resposta
      quanto na geração de embeddings.
...
# Retry para erros transitórios da Gemini (503 Service Unavailable, 429 RESOURCE_EXHAUSTED).
...
def _gemini_http_error(exc: Exception, what: str) -> HTTPException:
    ...
    if code == 503 or "UNAVAILABLE" in msg:
        return HTTPException(
            status_code=502,
            detail=f"{what}: a Gemini está temporariamente sobrecarregada (503). Tente novamente em instantes.",
        )
    if code == 404:
...
def _is_retryable_gemini_error(exc: Exception) -> bool:
    """503 (sobrecarga temporária) e 429 (limite de taxa) valem uma nova tentativa."""
    code = getattr(exc, "code", None)
    msg = str(exc)
...
async def _retry_delay(attempt: int) -> None:
    """Backoff exponencial: 1.5s, 3s, 6s... (com GEMINI_RETRY_BASE_DELAY padrão)."""
...
async def embed_text(text: str, task_type: str = "RETRIEVAL_DOCUMENT") -> List[float]:
    ...
    Repete a chamada (com backoff exponencial) se a Gemini responder com um erro
    transitório (503/429), antes de desistir e devolver um erro amigável.
    """
    ...
    for attempt in range(GEMINI_MAX_RETRIES + 1):
        try:
            ...
            if _is_retryable_gemini_error(exc) and attempt < GEMINI_MAX_RETRIES:
                logger.warning(
                    "Embedding falhou (tentativa %s/%s, erro transitório); tentando de novo...",
                    attempt + 1,
                    GEMINI_MAX_RETRIES,
                )
...
async def generate_answer(system_prompt: str, history: List[Dict[str, str]], user_message: str) -> str:
    ...
    Se a Gemini responder com um erro transitório (503 Service Unavailable ou
    429 RESOURCE_EXHAUSTED), tenta de novo com backoff exponencial antes de
    desistir e propagar um erro amigável ao usuário. Isso evita que picos
    momentâneos de indisponibilidade da API (comuns em modelos "-latest")
    virem um 502 Bad Gateway direto para quem está conversando.
    """
    if gemini_client is None:
        raise HTTPException(status_code=500, detail="Gemini não configurado (GEMINI_API_KEY ausente).")
    ...
    for attempt in range(GEMINI_MAX_RETRIES + 1):
        try:
            answer = await asyncio.to_thread(_generate_sync, model, system_prompt, history, user_message)
            return answer or "Não consegui gerar uma resposta agora. Pode reformular a pergunta?"
        except Exception as exc:  # noqa: BLE001
            last_exc = exc
            if _is_retryable_gemini_error(exc) and attempt < GEMINI_MAX_RETRIES:
                logger.warning(
                    "Gemini indisponível ao gerar resposta (tentativa %s/%s, modelo=%s); tentando de novo...",
                    attempt + 1,
                    GEMINI_MAX_RETRIES,
                    model,
                )
    ...
    # Nunca deveria chegar aqui, mas por segurança:
    raise _gemini_http_error(last_exc or RuntimeError("erro desconhecido"), "Falha ao gerar a resposta")
