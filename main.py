...
app = FastAPI(title="RAG Backend", version="1.2.0")
...
def build_system_prompt(context_chunks: List[Dict[str, Any]]) -> str:
    ...
    Regra:
      - Se houver chunks com score >= QDRANT_SCORE_THRESHOLD, o modelo responde
        com base neles, mantendo um tom conversacional.
      - Se NÃO houver contexto relevante, ele conversa normalmente (saudações, papo casual)
        e só admite que não tem a informação quando a pergunta pede algo factual/específico
        da base de conhecimento.
      - Em ambos os casos usa o histórico da conversa para manter o assunto.
    """
    relevant_chunks = [c for c in context_chunks if c.get("score", 0) >= QDRANT_SCORE_THRESHOLD]

    common_rules = (
        "Use o histórico da conversa para manter o assunto: entenda referências como 'isso', "
        "'e o outro?' ou 'explica melhor' com base nas mensagens anteriores.\n"
        "Responda sempre no mesmo idioma do usuário.\n"
    )

    if relevant_chunks:
        context_text = "\n\n".join(f"[Fonte: {c['title']}]\n{c['text']}" for c in relevant_chunks)
        return (
    ...
    return (
        ...
        "'obrigado'), ou uma continuação do assunto que já estava sendo conversado, responda "
        "normalmente, de forma humana, breve e cordial.\n"
